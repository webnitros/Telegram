...
Telegram.utils.formatDate = function (string) {
    if (string && string != '0000-00-00 00:00:00' && string != '-1-11-30 00:00:00' && string != 0) {
        ...
        return date.strftime(Telegram.config['date_format']);
    }
    else {
        return '&nbsp;';
    }
...
Telegram.utils.renderBoolean = function (value) {
    return value
        ? String.format('<span class="green">{0}</span>', _('yes'))
        : String.format('<span class="red">{0}</span>', _('no'));
};

Telegram.utils.getMenu = function (actions, grid, selected) {
    var menu = [];
...
Telegram.utils.renderActions = function (value, props, row) {
    var res = [];
    ...
    for (var i in row.data.actions) {
        ...
        item = String.format(
            '<li class="{0}"><button class="telegram-btn telegram-btn-default {1}" action="{2}" title="{3}"></button></li>',
    ...
    return String.format(
        '<ul class="telegram-row-actions">{0}</ul>',
        res.join('')
    );
};


Telegram.utils.userLink = function (value, id, blank) {
    if (!value) {
        return '';
    }
    else if (!id) {
